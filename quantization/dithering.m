...
%% Dithering - adding 5 bits of noise
dithered = zeros(4000,1);
places = pow2(-(1:11));

for m = 1:4000
    noise = (-1)^round(rand) * rand * 31/2^11;
    dithered(m,:) = (-1)^unif_binary(m,1)*sum(unif_binary(m,2:end).*places,2)+noise;
end


%% Convert to binary
dith_binary = zeros(4000,12);
for m = 1:4000
    val = abs(dithered(m,1));
    for i=1:precision-1
        dith_binary(m,i+1) = floor(val*2);
        val = val*2-floor(val*2);
    end
    if (sign(unif_quant(m))==-1)
        dith_binary(m,1)=1;
...
figure;
stem(f,amp_fft_xu);
title('Amplitude spectrum for dithered and truncated signal','fontsize',24);
